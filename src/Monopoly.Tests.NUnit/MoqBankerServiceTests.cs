...
{
    [TestFixture]
    class Using_moq_when_the_bank_pays_a_player
    {
        private Player _player;
        private BankerService _service;
        private Mock<IBankRepository> _mock;

        [SetUp]
        public void before_each()
        {
            _player = new Player { AccountBalance = 875 };
            _mock = new Mock<IBankRepository>();
            _mock.Setup(m => m.TakeMoneyFromTheBank(It.IsAny<int>())).Returns(true);
            _mock.Setup(m => m.TakeMoneyFromTheBank(550)).Returns(false);
            _service = new BankerService(_mock.Object);
        }
    ...
    public class Using_moq_when_the_player_pays_the_bank_and_has_enough_money
    {
        private Player _player;
        private BankerService _service;
        private Mock<IBankRepository> _mock;
        ...
        public void before_each()
        {
            ...
            _mock = new Mock<IBankRepository>();
            _mock.Setup(m => m.PutMoneyInTheBank(It.IsAny<int>()));
            _service = new BankerService(_mock.Object);
        ...
        {
            _mock.Verify(m => m.PutMoneyInTheBank(It.IsAny<int>()), Times.AtLeastOnce());
        }
    }

    public class Using_moq_when_the_player_pays_the_bank_and_does_not_have_enough_to_cover_the_bill
    {
        private Player _player;
        private BankerService _service;
        private Mock<IBankRepository> _mock;
        ...
        {
            _mock.Verify(m => m.PutMoneyInTheBank(It.IsAny<int>()), Times.Never());
        }
    }
}
